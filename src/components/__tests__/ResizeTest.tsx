        ...
        <div
          className={clsx(
            ...
            metrics.averageProcessingTime > 16
              ? 'text-red-300'
              : metrics.averageProcessingTime > 8
                ? 'text-yellow-300'
                : 'text-green-300'
          )}
        >
  ...
  return (
    <div className="absolute top-4 left-4 w-80 bg-black/80 backdrop-blur-md rounded-lg p-4 text-white text-sm z-50">
      ...
      <div className="space-y-2">
        <p>
          <strong>🔄 Basic Resizing:</strong> Drag resize handles on panel edges/corners
        </p>
        <p>
          <strong>📐 Aspect Ratio:</strong> Hold Shift while resizing to lock ratio
        </p>
        <p>
          <strong>📏 Constraints:</strong> Min/max size limits are enforced
        </p>
        <p>
          <strong>👁️ Preview:</strong> Live dimension display during resize
        </p>
        <p>
          <strong>🎯 Snap Zones:</strong> Panels snap to common sizes
        </p>
        <p>
          <strong>🚫 Collision:</strong> Panels prevent overlapping
        </p>
        <p>
          <strong>👥 Multi-Panel:</strong> Select multiple panels (Ctrl+Click)
        </p>
        <p>
          <strong>⚡ Performance:</strong> Monitor frame rate in top-right
        </p>
      </div>
        ...
        <div className="space-y-2">
          ...
          <button
            ...
            className={clsx(
              ...
              constraintsEnabled ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
            )}
          >
          ...
          <button
            ...
            className={clsx(
              'w-full px-3 py-2 text-white text-sm rounded transition-colors',
              previewEnabled ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
            )}
          >
    ...
    >
      <div className="space-y-2 text-white">
        ...
        <div className="text-sm space-y-1">
          <div>
            Size: {Math.round(panel.size.width)} × {Math.round(panel.size.height)}
          </div>
          <div>
            Position: ({Math.round(panel.position.x)}, {Math.round(panel.position.y)})
          </div>
...
export const ResizeTest: React.FC = () => {
  ...
  const handlePanelSelect = useCallback((panelId: string, multiSelect: boolean) => {
    ...
    setPanels(prev =>
      prev.map(p => ({
        ...p,
        selected: multiSelect ? (p.id === panelId ? !p.selected : p.selected) : p.id === panelId,
      }))
    );
  }, []);
  ...
  const handleResize = useCallback(
    (panelId: string, newSize: Size) => {
      ...
      if (constraintsEnabled) {
        setPanels(prev =>
          prev.map(p => (p.id === panelId ? { ...p, size: validation.validatedSize } : p))
        );
      } else {
        setPanels(prev => prev.map(p => (p.id === panelId ? { ...p, size: newSize } : p)));
      }
    },
  ...
  return (
    <div className="relative w-full h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-blue-900 overflow-hidden">
      ...
      {panels.map(panel => (
        ...
        >
          <TestPanelContent panel={panel} onSelect={handlePanelSelect} />
        </ResizablePanel>
      ))}
