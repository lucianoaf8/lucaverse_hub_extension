...
export const useMultiPanelResize = (
  ...
  const calculateProportionalResize = useCallback(
    (
      ...
      const rightAdjacent = adjacentPanels.filter(
        p =>
      ...
      const bottomAdjacent = adjacentPanels.filter(
        p =>
        ...
        for (const otherPanel of otherPanels) {
          const overlap = {
            x: Math.max(
              0,
              Math.min(
                position.x + update.newSize.width,
                otherPanel.position.x + otherPanel.size.width
              ) - Math.max(position.x, otherPanel.position.x)
            ),
            y: Math.max(
              0,
              Math.min(
                position.y + update.newSize.height,
                otherPanel.position.y + otherPanel.size.height
              ) - Math.max(position.y, otherPanel.position.y)
            ),
          };
  ...
  const queueResizeOperation = useCallback(
    (panelId: string, targetSize: Size, constraints: ResizeConstraints, priority: number = 1) => {
  ...
  }, [resizeQueue, finalConfig.maxOperationsPerFrame, resolveConflicts, panels, onPanelUpdate]);
