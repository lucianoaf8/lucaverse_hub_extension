...
export function usePanelDrag(panelId: string) {
  ...
  return {
    ...
    dragOffset,
    handleDragStart,
    currentPosition: dragState.isDragging && dragState.draggedPanelId === panelId 
...
export function usePanelResize(panelId: string) {
  ...
  const handleResizeMove = useCallback(
    throttle((event: MouseEvent) => {
      ...
      if (resizeDirection.includes('n') && constrainedSize.height !== newSize.height) {
        constrainedPosition.y = resizeStartData.current.position.y + 
      ...
      if (resizeDirection.includes('w') && constrainedSize.width !== newSize.width) {
        constrainedPosition.x = resizeStartData.current.position.x + 
  ...
  return {
    ...
    resizeDirection,
    handleResizeStart,
    currentSize: resizeState.isResizing && resizeState.resizedPanelId === panelId 
      ? resizeState.currentSize 
      : undefined
  };
}
...
export function usePanelSelection(panelId: string) {
  ...
  const handleSelect = useCallback((multiSelect = false) => {
  ...
  const handleToggleSelect = useCallback((multiSelect = false) => {
    if (isSelected) {
      handleDeselect();
    } else {
...
export function useGridSnap() {
  ...
  const shouldSnap = useCallback((position: Position): boolean => {
